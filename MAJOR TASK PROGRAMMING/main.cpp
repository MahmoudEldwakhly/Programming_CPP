...
void get_user_input(int& numTellers, int& bankOpenTime, int& bankCloseTime, int& break_start_time, int& break_end_time, int& numCustomersPerday, int& numservedCustomersPerYear, int& numTransactions,  Transaction transactions[])
{
...
{
    const int NUM_DAYS = 1; // if we want calculations in more than one day
    ...
    for (int day = 1; day <= NUM_DAYS; day++)    // We can change it if we want to run the whole program in more than 1 day.
    {
        ...
        for (int i = 0; i < numCustomersPerday; i++)
        {
            ...
            prevTellerAssigned = assignedTeller;
             if (i<3) customers[i].waitingTime = 0;
             if (i>=3)
             {
            ...
            for (int j = i ; j > 2; j-- )
            {
             ...
             if (customers[i].tellerAssigned == customers[j].tellerAssigned &&  customers[i].arrivalTime < customers[j].leavingtime  )
            {
                customers[i].waitingTime  = fabs( customers[i].arrivalTime  - customers[j].arrivalTime - customers[j].serviceTime - customers[j].waitingTime);
            }
            else
           ...
           customers[i].leavingtime = leavingMinutes ;
...
*/
                 ...
                 << " and finishes at " << setw(2) << setfill('0') << leavingHours << ":" << setw(2) << leavingMinutes<<endl
...
{
    srand(time(0));
    int numTellers, bankOpenTime, bankCloseTime, numCustomersPerday, numservedCustomersPerYear, numTransactions, break_start_time, break_end_time;//break_time , wa

    cout << "How many transactions will you enter? ";
    cin >> numTransactions;
    Transaction transactions[numTransactions];
    get_user_input(numTellers, bankOpenTime, bankCloseTime, break_start_time, break_end_time, numCustomersPerday, numservedCustomersPerYear, numTransactions, transactions);
